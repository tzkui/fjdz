<template>
  ...
  <view class="fjyx" v-else-if="nowPage === 1">
    ...
    <view class="" v-else-if="gameState === 0">
      ...
      <div class="nd_desc">
        <img
          v-if="selectedDiff === difficultyList[0].val"
          src="../../static/imgs/medal-bronze1.png"
          alt=""
        />
        <img
          v-if="selectedDiff === difficultyList[1].val"
          src="../../static/imgs/medal-bronze2.png"
          alt=""
        />
        <img
          v-if="selectedDiff === difficultyList[2].val"
          src="../../static/imgs/medal-bronze3.png"
          alt=""
        />
        <img
          v-if="selectedDiff === difficultyList[3].val"
          src="../../static/imgs/medal-bronze4.png"
          alt=""
        />
        <div v-if="selectedDiff === difficultyList[0].val">
          适合刚开始学习入门指法的你
        </div>
        <div v-if="selectedDiff === difficultyList[1].val">
          适合学完指法进阶练习的你
        </div>
        <div v-if="selectedDiff === difficultyList[2].val">
          适合不看键盘开始逐步提速的你
        </div>
        <div v-if="selectedDiff === difficultyList[3].val">
          适合熟练掌握指法开始挑战自我的你
        </div>
      </div>
      <div class="nd">
        <ul>
          <li
            @click="selectedDiff = item.val"
            :class="item.val === selectedDiff ? 'active' : ''"
...
const getRandomEnemyX = () => {
  let x = parseInt(
    Math.random() *
      (window.innerWidth / 2 - playerPhaneSize.width / 2 - enemyPlaneSize.width)
  );
  if (Math.random() < 0.5) {
    return x;
  } else {
...
const drawBullet = (
  bullet = {
    ...
    y:
      ...
      playerPhaneSize.height -
      200 -
      bulletImgSize.height / 2,
    angle: 0,
  }
) => {
  ctx.save();
  ctx.translate(
    bullet.x + bulletImgSize.width / 2,
    bullet.y + bulletImgSize.height + 24,
  );
  ctx.rotate(bullet.angle + Math.PI / 2);
  ctx.drawImage(bulletImg, -bulletImgSize.width / 2, -bulletImgSize.height, bulletImgSize.width, bulletImgSize.height);
  // 恢复画布状态
  ctx.restore();
};
